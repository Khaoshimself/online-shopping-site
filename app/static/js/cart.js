...
document.addEventListener("DOMContentLoaded", () => {
  ...
  async function handleAddToCartClick(button, toast) {
    ...
    try {
      // This is the API call to our *temporary* backend
  ...
  async function loadCartTable(cartTableBody) {
    ...
    try {
      // Get the cart data from temporary API
      ...
      if (data.items.length === 0) {
        ...
        cartTableBody.innerHTML = data.items.map((item) => `
          <tr>
            <td>
      ...
      document.getElementById("cart-subtotal").textContent = `$${(data.subtotal_cents / 100).toFixed(2)}`;
      document.getElementById("cart-tax").textContent = `$${(data.tax_cents / 100).toFixed(2)}`;
      document.getElementById("cart-total").textContent = `$${(data.total_cents / 100).toFixed(2)}`;
  ...
  async function updateCartItem(productId, quantity, cartTableBody) {
    try {
      ...
      const data = await response.json();
      updateCartCountBadge(data.cart_item_count); // Update badge
    } catch (error) {
  ...
  function showNotification(toast, message, type = "success") {
    ...
    toastHeader.classList.add(type === "success" ? "bg-success" : "bg-danger", "text-white");
